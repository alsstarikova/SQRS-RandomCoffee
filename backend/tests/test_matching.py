...
def test_run_matching_creates_pairs(client, mailer_store, db_session):
    ...
    resp = client.post("/matching/run", headers=_headers(token_a))
...
def test_run_matching_not_enough_users(client, mailer_store):
    ...
    assert client.post("/matching/run", headers=_headers(token)).status_code == 400
...
def test_triple_all_three_see_two_partners(client, mailer_store, db_session):
    ...
    for token in (token_a, token_b, token_c):
        resp = client.get("/matching/my", headers=_headers(token))
        assert resp.status_code == 200
        matches = resp.json()
        assert len(matches) == 1
...
def test_get_my_matches_pair(client, mailer_store):
    ...
    resp = client.get("/matching/my", headers=_headers(token_a))
    assert resp.status_code == 200
    matches = resp.json()
    assert len(matches) == 1
